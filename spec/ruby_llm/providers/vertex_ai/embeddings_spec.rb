...
RSpec.describe RubyLLM::Providers::VertexAI::Embeddings do
  ...
  describe '#render_embedding_payload' do
    context 'with text only' do
      ...
      it 'merges other arguments into the payload' do
        payload = embeddings.send(
          :render_embedding_payload,
          'Hello!',
          model: 'multimodalembedding',
          dimensions: 512,
          with: 'gs://my-bucket/video.mp4',
          other: {
            videoSegmentConfig: {
              startOffsetSec: 0,
              endOffsetSec: 12,
        ...
        expect(payload[:instances].first[:text]).to eq('Hello!')
        expect(payload[:parameters]).to eq({ dimension: 512 })
        expect(payload[:instances].first[:video][:videoSegmentConfig]).to eq(
          '{"startOffsetSec":0,"endOffsetSec":12,"intervalSec":8}'
        )
      end
    end
    ...
    context 'with multimodal content payload' do
      ...
      it 'deep merges other arguments into multimodal payload instances' do
        payload = embeddings.send(
          :render_embedding_payload,
          'Test multimodal input',
          model: 'multimodalembedding',
        ...
        expect(payload[:instances].first[:video][:gcsUri]).to eq('gs://my-bucket/my-video.mp4')
        expect(payload[:instances].first[:video][:videoSegmentConfig]).to eq(
          {
            startOffsetSec: 0,
            endOffsetSec: 10,
